...
export class UsersController {
    ...
    async create(@Param('iduser') iduser: string, @Body() createuserdto: CreateUserDto, @Res() res): Promise<Users> {
        try {
            ...
            return res.status(HttpStatus.CREATED).json({ message: 'User created successfully', data: createUser,success:true });
        } catch (error) {
    ...
    async getAllByUser(@Param('iduser') iduser: string, @Res() res): Promise<Users[]> {
        try {
            ...
            return res.status(HttpStatus.OK).json({ message: 'List of users', data: existsUsers,success:true });
        } catch (error) {
    ...
    async getById(@Param('id') id: string, @Res() res): Promise<Users> {
        try {
            ...
            return res.status(HttpStatus.OK).json({ message: 'Data of user is:', data: existUser,success:true });
        } catch (error) {
            return res.status(HttpStatus.BAD_REQUEST).json({ message: 'Try again!', error: error.message });
        }
    }
    ...
    async update(@Body() updateuserdto: Updateuserdto, @Param('id') id: string, @Res() res): Promise<Users> {
        try {
            ...
            return res.status(HttpStatus.OK).json({ message: 'User updated successfully :)', data: updatedUser,success:true });
        } catch (error) {
            return res.status(HttpStatus.BAD_REQUEST).json({ message: 'Try again', error: error.message });
        }
    }
    ...
    async delete(@Param('iduser') iduser: string, @Param('id') id: string, @Res() res): Promise<Users> {
        try {
            ...
            return res.status(HttpStatus.OK).json({ message: 'User deleted successfully :)', data: deletedUser,success:true });
        } catch (error) {
            return res.status(HttpStatus.BAD_REQUEST).json({ message: 'Try again', error: error.message });
        }
    }
